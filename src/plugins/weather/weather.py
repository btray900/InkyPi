...
# GEOCODING_URL = "http://172.16.90.252:4444/geocode?api_key={api_key}"
...
class Weather(BasePlugin):
    ...
    def parse_open_meteo_forecast(self, daily_data, tz):
        ...
        for i in range(0, len(times)):
            ...
            try:
                ...
                moon = resp.json()[0]
                logger.info(f"BT106C Farmsense moon data: {moon}")
    ...
    def get_weather_data(self, api_key, units, lat, long):
        ...
        logging.info(f"BT106c Requesting weather data from URL: {url}")
    ...
    def get_open_meteo_data(self, lat, long, units, forecast_days):
        ...
        if not 200 <= response.status_code < 300:
            logging.error(
                f"Failed to retrieve Open-Meteo weather data: {response.content}"
            )
    ...
    def get_open_meteo_air_quality(self, lat, long):
        ...
        if not 200 <= response.status_code < 300:
            logging.error(
                f"Failed to retrieve Open-Meteo air quality data: {response.content}"
            )
